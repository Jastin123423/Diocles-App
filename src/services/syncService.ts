// src/services/syncService.ts (COMPLETE WITH SALE EDIT REQUESTS PULL + FORCE PULL)
...
export class SyncService {
  ...
  public static async processSyncQueue(
    ...
    // Skip only when NOT forced and nothing to sync
    if (!forcePull && !this.shouldSync() && !this.isSyncing) {
    ...
    try {
      ...
      // 2. PULL from cloud when there were pending items, local changes, OR forcePull
      if (pendingItems.length > 0 || this.hasLocalChanges || forcePull) {
